...
class DiConfigFilesTest extends \PHPUnit\Framework\TestCase
{
    ...
    protected function _prepareFiles()
    {
        ...
        $search = [];
        foreach ($configDirectory->search('{*/events.xml,events.xml}') as $path) {
        ...
        self::$_moduleGlobalFiles = $modulesReader->getConfigurationFiles('events.xml');

        //init module area configs
        $areas = ['adminhtml', 'frontend'];
        foreach ($areas as $area) {
            $moduleAreaFiles = $modulesReader->getConfigurationFiles($area . '/events.xml');
